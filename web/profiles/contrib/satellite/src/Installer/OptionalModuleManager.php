...
namespace Drupal\satellite\Installer;

use Drupal\Core\DependencyInjection\ContainerInjectionInterface;
use Drupal\Core\Extension\Extension;
use Drupal\Core\Extension\ModuleExtensionList;
use Drupal\Core\Serialization\Yaml;
use Drupal\satellite\Exception\OptionalFeatureDataException;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * Optional Module Manager.
 *
 * Used to discover extensions in ESN Satellite that can be optionally enabled
 * during installation.
 */
class OptionalModuleManager implements ContainerInjectionInterface {
  ...
  /**
   * Collects the optionally installable modules for ESN Satellite.
   *
   * A module is optionally installable if it contains a
   * `modulename.satellite_feature.yml` file. The file should contain data about
   ...
   * Helper function.
   *
   * Loads feature definitions from a `profile.installer_options_list.yml` in
   * the profile.
   *
   * This allows optional features to be defined for modules where it's not
   * possible to add a `modulename.satellite_feature.yml` file to the module.
   *
   * @return array
  ...
  protected function getInstallProfileFeatureDefinitions(): array {
    ...
    foreach ($features as $module_name => &$info) {
      ...
      try {
        $this->validateSatelliteFeatureData($module_name, $info);
      }
      catch (OptionalFeatureDataException $e) {
        throw new OptionalFeatureDataException("Invalid feature info for '{$module_name}' in `{$feature_list_name}`.", 0, $e);
      }
    }
  ...
  protected function getOptionalModuleInfo(Extension $extension): ?array {
    ...
    $optional_info_file = $module_directory . DIRECTORY_SEPARATOR . $extension->getName() . '.satellite_feature.yml';
    ...
    if (!$this->validateSatelliteFeatureData($extension->getName(), $feature_info)) {
      return NULL;
    }
  ...
  /**
   * Validate the data for an ESN Satellite optional feature.
   *
   ...
   *
   * @throws \Drupal\satellite\Exception\OptionalFeatureDataException
   *   An exception that indicates what's wrong with the data.
   */
  protected function validateSatelliteFeatureData(string $module_name, array $info): bool {
    if (!isset($info['name'])) {
      throw new OptionalFeatureDataException("Missing `name` field in `{$module_name}.satellite_feature.yml`.");
    }

    if (!isset($info['description'])) {
      throw new OptionalFeatureDataException("Missing `description` field in `{$module_name}.satellite_feature.yml`.");
    }

    if (!is_bool($info['default'])) {
      throw new OptionalFeatureDataException("Field `default` must be of type `bool` in `{$module_name}.satellite_feature.yml`.");
    }

    if (!is_int($info['weight'])) {
      throw new OptionalFeatureDataException("Field `weight` must be of type `int` in `{$module_name}.satellite_feature.yml`.");
    }
